...
unsigned char DisBuffer[16] = {0,0,0,0,0,0,0,0};
...
{
  unsigned char i;
  static unsigned char cmpDisBuffer[16] = {0};

  if (memcmp(cmpDisBuffer, DisBuffer, sizeof(DisBuffer)) == 0)
    return;
  memcpy(cmpDisBuffer, DisBuffer, sizeof(DisBuffer));
  ...
  for (i = 0; i < 16; i++)
  {
    send_shift(Table[DisBuffer[i^0x0F]]);
  }  
  NOP();
  NOP();
  send_storage_output();
